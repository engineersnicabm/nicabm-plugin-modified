...
class Shortcode {
	...
	protected function enqueue_scripts() {
		...
		foreach ( (array) $this->scripts as $script ) {
			...
			if ( $this->should_enqueue_style( $script ) ) {

				wp_enqueue_style( $args['handle'], $args['src'], $args['deps'], $args['ver'] );

			} elseif ( $this->should_enqueue_script( $script ) ) {
	...
	/**
	 * Determine if the asset is a CSS file and that it has not been enqueued already.
	 *
	 * @param array $script Script enqueue parameters.
	 * @return bool
	 */
	protected function should_enqueue_style( $script ) {
		return 'css' === substr( $script['src'], -3 ) && ! wp_style_is( $script['handle'] );
	}

	/**
	 * Determine if the asset is a JavaScript file and that it has not been enqueued already.
	 *
	 * @param array $script Script enqueue parameters.
	 * @return bool
	 */
	protected function should_enqueue_script( $script ) {
		return 'js' === substr( $script['src'], -2 ) && ! wp_script_is( $script['handle'] );
	}
